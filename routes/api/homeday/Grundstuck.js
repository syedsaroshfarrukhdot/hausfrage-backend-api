...
const { GrundstuckHomeDay } = require("../../../model/homeday/Grundstuck");
var checkSessionAuth = require("../../../middlewares/checkSessionAuth");
var auth = require("../../../middlewares/auth");

/*Get Projects*/
router.get("/", auth, async (req, res) => {
  let grundstuck = await GrundstuckHomeDay.find();
  res.send(grundstuck);
});

/* Add New Project . */
router.post("/create-form", async (req, res) => {
  grundstuck = new GrundstuckHomeDay(req.body);
  grundstuck
    .save()
    .then((resp) => {
...
router.put("/create-form-edit/:id", auth, async (req, res) => {
  try {
    let grundstuck = await GrundstuckHomeDay.findById(req.params.id);
